...
import { upsertAttendance, getStudentsForClass, getAttendanceForDate } from '../actions'
...
export function AttendanceClient({ classes }: { classes: Cls[] }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      atts.forEach(r => { map[r.student_id] = r.status as AttStatus })
  ...
  const setAll = (status: AttStatus) =>
    setRecords(prev => Object.fromEntries(Object.keys(prev).map(k => [k, status])))
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="shrink-0 px-4 sm:px-6 py-3 border-b border-gray-100 bg-white flex flex-wrap gap-3 items-center">
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => setDate(d => shiftDate(d, -1))} className="w-8 h-9 rounded-xl border border-gray-200 flex items-center justify-center text-gray-400 hover:text-[#1A2744] transition-colors"><ChevronLeft size={15} /></button>
          ...
          <button onClick={() => setDate(d => shiftDate(d, 1))} disabled={date >= today()} className="w-8 h-9 rounded-xl border border-gray-200 flex items-center justify-center text-gray-400 hover:text-[#1A2744] disabled:opacity-30 transition-colors"><ChevronRight size={15} /></button>
      ...
      {students.length > 0 && (
        <div className="shrink-0 px-4 sm:px-6 py-2 border-b border-gray-50 bg-[#F7F6F2] flex flex-wrap items-center gap-3">
          <div className="flex gap-3">
            {counts.map(a => <span key={a.v} className="text-xs font-semibold text-gray-500">{a.short} <span className="text-[#1A2744]">{a.count}</span></span>)}
          </div>
          <div className="ml-auto flex items-center gap-2">
            <span className="text-xs text-gray-400">Đánh dấu tất cả:</span>
            {ATT.map(a => (
              <button key={a.v} onClick={() => setAll(a.v)} className={`text-xs font-semibold px-2.5 py-1 rounded-full transition-all ${a.color} opacity-80 hover:opacity-100`}>
                {a.short} {a.l}
              </button>
            ))}
          </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-400 text-sm">{!classId ? 'Chọn lớp để điểm danh' : 'Lớp này chưa có học viên active'}</div>
        ) : (
